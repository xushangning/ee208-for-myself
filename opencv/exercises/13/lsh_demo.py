        ...
        for i, s in enumerate(self.map_sets):
            ...
            for c in self.table[i][lsh(v, s)]:
                ...
                for appended_candidate in candidates:
                    if np.array_equal(appended_candidate[0], c):
                        break
                else:
                    # second item is the Euclidean distance
                    candidates.append((c, np.sqrt(np.square(v.astype(np.float64) - c).sum())))
        candidates.sort(key=lambda c: c[1])
        if len(candidates) > k:
            return [candidates[i][0] for i in range(k)]
        else:
...
if __name__ == '__main__':
    ...
    for i in range(1, 41):
        filename = 'Dataset/{}.jpg'.format(i)
        img = cv.imread(filename)
        t.add(feature_vector(img))
